...
# Creator of CurriculumGenerator
...
#TODO one more thing your script is look like a simple python program. make it more attracive for UX if you've still any pls check this link :- "https://github.com/abhishekpatel946/PassManager-pyScript/tree/master/Screens" here is my script check the images on the link. thakns
...
cvStyle = random.choice(
    curriculumDataObj.cvStyle) if args.randomstyle else args.style
cvColor = random.choice(
    curriculumDataObj.cvColor) if args.randomcolor else args.color
...
if (personnae.email == None):
    defaultemail = personnae.firstname.lower(
    ) + "." + personnae.lastname.lower() + "@gmx.com"
...
# TODO ;- handle the edge case if user not enter any details like- "Number of Jobs elements ?" are not entered then script gives an error. I think you should consider an else statements to avoid those edge cases
...
if (personnae.webpage == None):
    personnae.webpage = "http://" + personnae.firstname.lower() + "." + \
        personnae.lastname.lower() + ".personnalbranding.com"
...
#TODO :- simply display the current time using ctime(), and correct the format of the `strftime()` time.
...
texSpecific = personnae.lastname + "." + \
    personnae.firstname  # + "_" + dt_string
...
shutil.copytree("../resources/latexSamples/img/",
                texcurriculumDirectory + "img/")

# Generate Makefile
print("Creating Makefile...")
with open(texcurriculumDirectory + "Makefile", 'w') as makefile:
    makefile.write(curriculumGeneration.getMakefileContent(
        texcurriculumFileName))
...
with open(texcurriculumDirectory + texcurriculumFileName + ".tex", 'w') as curriculumGenerationtest:
    curriculumGenerationtest.write(
        curriculumGeneration.getLaTeXHeaderPart1(cvColor, cvStyle))
    curriculumGenerationtest.write("\n\n")
    # Personnal Data
    ...
    curriculumGenerationtest.write(
        curriculumGeneration.getAddressDefinition(address=personnae.address) + "\n")
    curriculumGenerationtest.write(
        curriculumGeneration.getEMailDefinition(email=personnae.email) + "\n")
    curriculumGenerationtest.write(
        curriculumGeneration.getWebSiteDefinition(webpage=personnae.webpage) + "\n")
    curriculumGenerationtest.write(
        curriculumGeneration.getQuoteDefinition(quote=personnae.quote) + "\n")
    curriculumGenerationtest.write("\n\n")
    # more header
    curriculumGenerationtest.write(
        curriculumGeneration.getFancyStyle() + "\n\n")
    curriculumGenerationtest.write(
        "\\def\\motsClefs{LaTeX;PDF;Python;Python3...}\n\n")
    curriculumGenerationtest.write(
        curriculumGeneration.getHyperSetup() + "\n\n")
    curriculumGenerationtest.write(
        curriculumGeneration.getDefVariables() + "\n\n")
    ...
    for eltJOB in personnae.jobs:
        ...
        curriculumGenerationtest.write("\t \\cventry{years}{" + corporationName + " (" +
                                       eltJOB[1] + ")}{" + eltJOB[0] + "}{" + contractType + "}{\n %% grade \n}{\n %% description \n}\n\n")
    curriculumGenerationtest.write(
    ...
    for eltTraining in personnae.trainings:
        curriculumGenerationtest.write(
            "\t \\cventry{years}{ " + eltTraining[1] + " }{" + eltTraining[0] + "}{ LOCALISATION }{\n %% grade \n}{\n %% description \n}\n\n")
    curriculumGenerationtest.write(
...
if args.make:
    curriculumMainFunctions.launcheMakePDFfromLaTeX(
        directory=texcurriculumDirectory)
